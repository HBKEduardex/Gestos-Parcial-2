...
#include <std_msgs/msg/float32.h>   // <--- EXTRA: para /zone_dist_*
#include <string.h>
...
// EXTRA: suscripciones a distancias
rcl_subscription_t zone_left_sub;
rcl_subscription_t zone_center_sub;
rcl_subscription_t zone_right_sub;
...
// Mensajes para las zonas (Float32, sin memoria dinámica)
std_msgs__msg__Float32 zone_left_msg;
std_msgs__msg__Float32 zone_center_msg;
std_msgs__msg__Float32 zone_right_msg;

unsigned long last_cmd_ms = 0;
bool emergency_active = false;

// EXTRA: distancias en metros (valores por defecto grandes = sin obstáculo)
float zone_left_m   = 10.0f;
float zone_center_m = 10.0f;
float zone_right_m  = 10.0f;
...
// =========================
// EXTRA: AJUSTE POR OBSTÁCULOS
// =========================
void ajustar_velocidad_por_obstaculos()
{
  // Debug opcional:
  Serial.print("[ESP32] Zonas (m) L/C/R = ");
  Serial.print(zone_left_m, 2);
  Serial.print(" / ");
  Serial.print(zone_center_m, 2);
  Serial.print(" / ");
  Serial.println(zone_right_m, 2);

  // Regla frontal:
  // Si hay algo MUY cerca al frente (< 0.5 m) -> NO avanzar
  if (zone_center_m < 0.5f)
  {
    twist_msg.linear.x = 0.0f;
  }
  // Si hay algo entre 0.5 y 1.0 m al frente -> reducir velocidad hacia adelante
  else if (zone_center_m < 1.0f && twist_msg.linear.x > 0.0f)
  {
    twist_msg.linear.x *= 0.5f;
  }

  // Regla lateral izquierda:
  // Si giramos a la izquierda (angular.z > 0) y hay obstáculo a la izquierda
  if (zone_left_m < 0.7f && twist_msg.angular.z > 0.0f)
  {
    twist_msg.angular.z *= 0.5f;
  }

  // Regla lateral derecha:
  // Si giramos a la derecha (angular.z < 0) y hay obstáculo a la derecha
  if (zone_right_m < 0.7f && twist_msg.angular.z < 0.0f)
  {
    twist_msg.angular.z *= 0.5f;
  }
}
...
void gesture_callback(const void * msgin)
{
  ...
  Serial.print("[ESP32] Interpretado comando base: ");
  Serial.println(cmd);

  // actualizar timer de seguridad
  last_cmd_ms = millis();

  // generar twist base + leds
  fill_twist_for_text(cmd);
  setLedsForCommand(cmd);

  // EXTRA: ajustar twist según distancias
  ajustar_velocidad_por_obstaculos();
...
// =========================
// CALLBACKS de /zone_dist_*
// =========================
void zone_left_callback(const void * msgin)
{
  const std_msgs__msg__Float32 * msg = (const std_msgs__msg__Float32 *) msgin;
  zone_left_m = msg->data;
}

void zone_center_callback(const void * msgin)
{
  const std_msgs__msg__Float32 * msg = (const std_msgs__msg__Float32 *) msgin;
  zone_center_m = msg->data;
}

void zone_right_callback(const void * msgin)
{
  const std_msgs__msg__Float32 * msg = (const std_msgs__msg__Float32 *) msgin;
  zone_right_m = msg->data;
}
...
void setup() {
  ...
  // Executor
  // Antes teníamos 2 (gesture + timer). Ahora son 5: gesture + 3 zonas + timer.
  ...
  Serial.println("micro-ROS gesture controller con EXTRA inicializado!");
}
